...
const ampPlugin = async (html, route) => {
  try {
    ...
    link.setAttribute('href', `https://mirzailhami.com${route.route}/amp`);
    original.window.document.head.append(link);
    document.querySelector(
      'link[rel=canonical]'
    ).href = `https://mirzailhami.com${route.route}`;
    document.querySelector("script[type='application/ld+json']").innerHTML = `
    {
      ...
      "image": ["https://mirzailhami.com/assets/banners${route.route}/og.png"]
    }
    `;

    document.querySelector('.logo').href = `https://mirzailhami.com${route.route}`;
    document.querySelector('.more').href = `https://mirzailhami.com${route.route}`;
    document
      .querySelector('.og-image')
      .setAttribute(
        'src',
        `https://mirzailhami.com/assets/banners${route.route}/og.png`
      );
    document.querySelector('.more').innerHTML = `See related articles`;
    document
      .querySelector('.home-button')
      .setAttribute('src', 'https://mirzailhami.com/assets/img/logo-text.svg');
    ...
    imgs.forEach(i => {
      ...
      amp.setAttribute(
        'src',
        i.src.indexOf('https://') === -1 ? `https://mirzailhami.com/${i.src}` : i.src
      );
